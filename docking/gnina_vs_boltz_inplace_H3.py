...
r, p = pearsonr(df["Vina_Score_H3"], df["Boltz_Affinity_H3"])

# Plot Vina vs Boltz Affinity
plt.figure(figsize=(10, 5))

# Use diverging colormap with Boltz_Probability_H3
scatter = plt.scatter(df["Vina_Score_H3"], df["Boltz_Affinity_H3"], 
                     c=df["Boltz_Probability_H3"], cmap='coolwarm_r', alpha=0.9, s=30,
                     vmin=0, vmax=1)

plt.xlabel("In Place Vina Affinity (kcal/mol)")
plt.ylabel("Boltz-2 Affinity (kcal/mol)")
plt.title("Boltz-2 vs. Vina Affinity Estimates (Whole Complex)")
plt.grid(True)
...
y_min, y_max = df["Boltz_Affinity_H3"].min(), 0
